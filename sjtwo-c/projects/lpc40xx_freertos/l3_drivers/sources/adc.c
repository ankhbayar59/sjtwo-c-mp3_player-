...
void adc__enable_burst_mode(void) {
  const uint32_t channel_mask = 0xFF;
  const uint32_t start_conversion_mask = (7 << 24);
  const uint32_t burst_mode_mask = (1U << 16);
  const uint32_t selected_channel = (1U << 5);
  LPC_ADC->CR &= ~(channel_mask | start_conversion_mask);
  LPC_ADC->CR |= selected_channel;
  LPC_ADC->CR |= burst_mode_mask;
}

uint16_t adc__get_channel_reading_with_burst_mode() {
  uint16_t result = 0;
  uint16_t twelve_bits = 0x0FFF;
  while (!(LPC_ADC->DR[5] & (1U << 31))) {
  }
  result = (LPC_ADC->DR[5] >> 4) & twelve_bits;
  return result;
}