...
bool is_memory_address = false; // additional global variable I created in i2c.c

uint8_t memory_address; // additional global variable I created in i2c.c
...
static bool i2c__handle_state_machine(i2c_s *i2c) {
  enum {
    ...
    I2C__STATE_SR_SLAVE_ACK_SLA_W = 0x60,
    I2C__STATE_SR_SLAVE_READ_DATA = 0x80,
    I2C__STATE_SR_MASTER_MULTI_START = 0xA0,

    // Slave Transmitter States (ST):
    I2C__STATE_ST_SLAVE_ACK_SLA_R = 0xA8,
    I2C__STATE_ST_MASTER_ACK = 0xB8,
    I2C__STATE_ST_MASTER_NACK = 0xC0,
  };
  ...
  case I2C__STATE_SR_SLAVE_ACK_SLA_W:
    i2c__set_ack_flag(lpc_i2c);
    i2c__clear_si_flag_for_hw_to_take_next_action(lpc_i2c);
    break;

  case I2C__STATE_SR_SLAVE_READ_DATA:
    if (is_memory_address) {
      i2c__set_ack_flag(lpc_i2c);
      i2c__clear_si_flag_for_hw_to_take_next_action(lpc_i2c);
    } else {
      if (i2c_slave_callback__write_memory(memory_address, lpc_i2c->DAT)) {
        i2c__set_ack_flag(lpc_i2c);
        i2c__clear_si_flag_for_hw_to_take_next_action(lpc_i2c);
      } else {
  ...
  case I2C__STATE_SR_MASTER_MULTI_START:
    i2c__set_ack_flag(lpc_i2c);
    i2c__clear_si_flag_for_hw_to_take_next_action(lpc_i2c);
    break;

  case I2C__STATE_ST_SLAVE_ACK_SLA_R:
    if (true) {
      uint8_t memory;
      if (i2c_slave_callback__read_memory(memory_address, &memory)) {
        lpc_i2c->DAT = memory;
        i2c__set_ack_flag(lpc_i2c);
        i2c__clear_si_flag_for_hw_to_take_next_action(lpc_i2c);
      } else {
  ...
  case I2C__STATE_ST_MASTER_ACK:
    if (true) {

      uint8_t memory;
      if (i2c_slave_callback__read_memory(i2c->starting_slave_memory_address, &memory)) {
        i2c__set_ack_flag(lpc_i2c);
        i2c__clear_si_flag_for_hw_to_take_next_action(lpc_i2c);
        lpc_i2c->DAT = memory;
      } else {
    ...
    break;
  case I2C__STATE_ST_MASTER_NACK:
    i2c__set_ack_flag(lpc_i2c);
    i2c__clear_si_flag_for_hw_to_take_next_action(lpc_i2c);
    break;
  ...
  case I2C__STATE_MT_SLAVE_ADDR_ACK:
    ...
    if (0 == i2c->number_of_bytes_to_transfer) {
      ...
      memory_address = i2c->starting_slave_memory_address;
